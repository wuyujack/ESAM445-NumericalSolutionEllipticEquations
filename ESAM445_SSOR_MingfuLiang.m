function ESAM445_SSOR_MingfuLiang(K,N,omega,starting_method)
% Author: Mingfu Liang
% Date: 2019/05/07
%
% Implementation of SSOR Method for solving Helmholtz equation.
%
% Input:
%          K: 
...
%                overrelaxation.
%                   
%               The weight parameter for SSOR method.
%
%          starting_method: 
...
% Example:
%
%           ESAM445_SSOR_MingfuLiang(-2,16,1.8,1) 
%           
%           means that you are going to set the grid size N=16 and omega
%           =1.8 to do the SSOR method for Helmholtz equation 
...
while 1
    ...
    %%% SSOR Implementation. Here in SSOR, the update of each individual
    ...
    %%% the solution and also satisfies the requirement of SSOR. To
    %%% satisfy the requirement of the SSOR, I sweep the grid backward such
    %%% that it remove the asymmetry from SOR.
...
mesh(X,Y,u);
title1 = ['K=', num2str(K), ',omega=',num2str(w),',N=',num2str(N), ',SSOR Solution at ',num2str(loop_count),' iteration, starting data ', starting_name];
title(title1)
toc
end