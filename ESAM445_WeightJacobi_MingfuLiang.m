...
% Example:
%
%           ESAM445_WeightJacobi_MingfuLiang(-2,16,2/3,1) 
%           
%           means that you are going to set the grid size N=16 and omega
%           =2/3 to do the Weight Jacobi method for Helmholtz equation 
...
while 1
    ...
    %%% Weight Jacobi Implementation. Here I parallel all the calculation at the
    %%% same time since in Weight Jacobi the update of each individual
    ...
    u_update(i,j) = (1-w)*(u(i,j))+ ...
                     w*(1/(4 - (h^2)*pi*pi*K))*( u(i-1,j) + u(i+1,j) + ...
...
mesh(X,Y,u);
title1 = ['K=', num2str(K), ',omega=',num2str(w),',N=',num2str(N), ',Weight Jacobi Solution at ',num2str(loop_count),' iteration, starting data ', starting_name];
title(title1)
toc
end